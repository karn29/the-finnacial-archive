...
export default function App() {
  ...
  const advanceMonth = () => {
    ...
    Object.keys(newHoldings).forEach(name => {
      ...
      if (company && sector) {
        ...
        const eventImpact = (activeEvent?.sectorImpacts[sector.name] || 1) - 1;
  ...
  const portfolioPieData = {
    labels: Object.keys(allocations).length > 0 ? Object.keys(allocations) : ['Unallocated'],
    datasets: [{
  ...
  return (
    <div className="min-h-screen max-w-[1100px] mx-auto px-6 py-12">
      ...
      <main>
          ...
          {currentModule === 'archive' && (
            ...
            >
              ...
              {scenariosData.map((scenario) => (
                <div 
                  key={scenario.id}
                  onClick={() => handleScenarioSelect(scenario as any)}
                  className="newspaper-card p-8 cursor-pointer flex flex-col justify-between group h-full"
                >
                  <div>
                    ...
                    <h3 className="text-2xl mb-3">{scenario.name}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 column-rule">
                  <div>
                    ...
                    <p className="text-lg italic leading-relaxed">
                      {getMacroInsight(selectedScenario.macro)}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-2xl mb-4 flex items-center gap-2"><Zap className="w-6 h-6 text-sepia" /> Government Policy</h3>
                    <p className="text-lg italic leading-relaxed">
                      Policy makers focused on {selectedScenario.macro.inflation[0] > 5 ? 'curbing inflation' : 'stimulating growth'}, leading to significant structural shifts in the {selectedScenario.sectors[0].name} sector.
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {sector.companies.map(c => (
                            <button 
                              key={c.name} 
                              onClick={() => setExpandedCompany(c.name)}
                              className="border border-dark-sepia px-2 py-1 text-xs bg-parchment hover:bg-ink hover:text-parchment transition-colors flex items-center gap-1"
                            >
                      ...
                      <div key={company.name} className="newspaper-card p-4">
                        <div className="flex justify-between items-center mb-2">
                          <span className="font-bold text-ink flex items-center gap-2">
                            {company.name}
                            <button onClick={() => setExpandedCompany(company.name)} className="text-[10px] uppercase font-mono border border-ink px-1 hover:bg-ink hover:text-parchment">Profile</button>
                          </span>
                          <span className="text-xs opacity-80 text-sepia">{company.sector}</span>
                        </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <h3 className="text-5xl mt-2 leading-[0.9] tracking-tighter uppercase mb-8 border-b border-tan-mid pb-6">
                          {selectedScenario.months[currentMonthIndex]?.headline || "Market Continues Steady Progression"}
                        </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                          <div className="md:col-span-3">
                            <p className="text-2xl font-serif italic leading-tight opacity-90 first-letter:text-6xl first-letter:font-bold first-letter:mr-3 first-letter:float-left">
                              {selectedScenario.months[currentMonthIndex]?.description || "The global financial markets continue to react to the ongoing economic climate. Investors remain focused on long-term indicators as the simulation progresses through this historical period."}
                            </p>
                          </div>
                  ...
                  {selectedScenario.months[currentMonthIndex]?.lesson && (
                    ...
                    >
                      ...
                      <p className="text-sm opacity-80">{selectedScenario.months[currentMonthIndex].lesson?.text}</p>
                    </motion.div>
                  )}
                ...
                <div className="space-y-8">
                  ...
                  <div className="bg-ink text-parchment p-8 shadow-[4px_4px_0px_var(--sepia)] space-y-6">
                    ...
                    <div className="h-32 pt-4">
                      <Pie 
                        data={{
                          labels: ['Cash', ...Object.keys(currentHoldings)],
                          datasets: [{
          ...
          {step === 'result' && selectedScenario && (
            extendedScenarioData ? (
              ...
              >
                ...
                <div className="flex justify-center pt-10">
                  <button onClick={resetSimulation} className="bg-ink text-parchment px-12 py-4 font-bold text-xl hover:bg-ink/90 transition-all flex items-center gap-3">
                          ...
                          {quizAnswered !== null && (
                            <div className="mt-6 pt-6 border-t border-parchment/20 text-center">
                              <p className={`font-bold text-lg mb-4 ${quizAnswered === selectedTopic.quiz.answer ? 'text-sepia' : 'text-rust'}`}>
                                {quizAnswered === selectedTopic.quiz.answer ? 'Correct!' : 'Incorrect. Review the theory and try again.'}
                              </p>
                              <button 
                                onClick={() => {
        ...
        {expandedCompany && selectedScenario && (
          <CompanyProfileModal 
            company={selectedScenario.sectors.flatMap(s => s.companies).find(c => c.name === expandedCompany)}
