...
export const PostGameDebrief: React.FC<PostGameDebriefProps> = ({
    ...
    datasets: [
      {
        ...
        backgroundColor: allResults.map(r => r.isUser ? 'var(--sepia)' : 'var(--dark-sepia)'),
        borderColor: allResults.map(r => r.isUser ? 'var(--sepia)' : 'var(--dark-sepia)'),
        borderWidth: 1,
      }
    ]
  };
  ...
  const chartOptions = {
    ...
    scales: {
      y: {
        grid: { color: 'var(--tan-mid)' },
        ticks: { color: 'var(--ink)', font: { family: 'Courier New' } }
      },
      x: {
        grid: { display: false },
        ticks: { color: 'var(--ink)', font: { family: 'Playfair Display' } }
      }
    }
  };
